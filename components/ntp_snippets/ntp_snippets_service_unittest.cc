...
TEST_F(NTPSnippetsServiceTest, ImageReturnedWithTheSameId) {
  ...
  gfx::Image image;
  MockFunction<void(const std::string&, const gfx::Image&)> image_fetched;
  {
    InSequence s;
    EXPECT_CALL(*image_fetcher(), StartOrQueueNetworkRequest(_, _, _))
        .WillOnce(WithArgs<0, 2>(Invoke(ServeOneByOneImage)));
    EXPECT_CALL(image_fetched, Call(_, _)).WillOnce(SaveArg<1>(&image));
  }

  service->FetchSuggestionImage(
      MakeUniqueID(*service, kSnippetUrl),
      base::Bind(&MockFunction<void(const std::string&,
