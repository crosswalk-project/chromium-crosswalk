...
{ 'return_type': 'GLenum',
  ...
  'arguments': 'void',
  'logging_code': """
  GL_SERVICE_LOG("GL_RESULT: " << gpu::gles2::GLES2Util::GetStringError(result));
""", },
{ 'return_type': 'void',
...
""" % set_name.upper())
  ...
  def GetGLVersionCondition(gl_version):
    if GLVersionBindAlways(gl_version):
     ...
     if gl_version.minor_version == 0:
       return 'ver->is_es%d' % gl_version.major_version
     else:
       return 'ver->is_es%d%d' % (gl_version.major_version, gl_version.minor_version)
    else:
      return '!ver->is_es'
  ...
  for func in functions:
    ...
    log_argument_names = re.sub(
        r'GLenum_([a-zA-Z0-9_]+)', r'gpu::gles2::GLES2Util::GetStringEnum(\1)',
        log_argument_names)
    log_argument_names = re.sub(
        r'GLboolean_([a-zA-Z0-9_]+)', r'gpu::gles2::GLES2Util::GetStringBool(\1)',
        log_argument_names)
...
def GenerateEnumUtils(out_file, input_filenames):
  ...
  out_file.write("static const gpu::gles2::GLES2Util::EnumToString "
                 "enum_to_string_table[] = {\n")
  for value in dict:
    out_file.write('  { %s, "%s", },\n' % (value, dict[value]))
  out_file.write("""};

const gpu::gles2::GLES2Util::EnumToString* const GLEnums::enum_to_string_table_ =
  enum_to_string_table;
const size_t gpu::gles2::GLES2Util::enum_to_string_table_len_ =
...
def main(argv):
  ...
  def ClangFormat(filename):
    formatter = "buildtools/linux64/clang-format"
  ...
  for [functions, set_name, extension_headers, extensions] in FUNCTION_SETS:
    ...
    header_file = open(
        os.path.join(directory, 'gl_bindings_autogen_%s_android.h' % set_name), 'wb')
    GenerateHeader(header_file, functions, set_name, used_extensions)
    header_file.close()
    ClangFormat(header_file.name)

    source_file = open(
        os.path.join(directory, 'gl_bindings_autogen_%s_android.cc' % set_name), 'wb')
