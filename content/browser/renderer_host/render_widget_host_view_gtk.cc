...
static WebKit::WebMouseEvent latest_mouse_down_event;
...
bool shouldForgetPreviousClick(double time, double x, double y)
{
    ...
    double double_click_distance = 5;
...
class RenderWidgetHostViewGtkWidget {
 ...
 private:
  ...
  static GdkFilterReturn TouchEventPreFilter(GdkXEvent* xevent,
    ...
    RenderWidgetHostViewGtk* host_view = (RenderWidgetHostViewGtk*) data;
    ...
    switch(xev->xgeneric.evtype) {
      ...
      case XI_TouchEnd: {
        ...
        Window wid = GDK_WINDOW_XID(
                         GTK_WIDGET(host_view->GetNativeView())->window);
        if (wid != xiev->event)
          return GDK_FILTER_CONTINUE;
        ...
        if (xev->xgeneric.evtype == XI_TouchBegin) {
          ...
          if (touch_point == 0 &&
              static_cast<int>(xiev->event_x) == latest_mouse_down_event.x &&
              static_cast<int>(xiev->event_y) == latest_mouse_down_event.y) {
